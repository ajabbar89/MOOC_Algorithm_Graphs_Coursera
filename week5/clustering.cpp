...
double kruskals(std::vector<int> &x, std::vector<int> &y, int n, int k) {
	...
	for(i = 0; i < edges.size(); i++) {
		...
		if(union_num > (n-k))
			return edges[i].dist; //Halting the algorithm when there are k connected components and the last k-1 edges are not added
	}
	return edges[i-1].dist;
}
...
int main() {
	...
	int k;
	std::cin>>k;
	double distance = kruskals(x,y,n,k);
